...
pub fn parse_nginx(line: &str) -> HashMap<String, String> {
    parse_http_like("nginx", line, 0.9)
}

pub fn parse_apache(line: &str) -> HashMap<String, String> {
    parse_http_like("apache", line, 0.9)
}
...
pub fn parse_json(line: &str) -> HashMap<String, String> {
    match serde_json::from_str::<Value>(line) {
        Ok(json) => {
            ...
            row.insert("service".to_string(), json.get("service").and_then(|v| v.as_str()).unwrap_or("unknown").to_string());
            row.insert("level".to_string(), json.get("level").and_then(|v| v.as_str()).unwrap_or("info").to_string());
            row.insert("message".to_string(), json.get("message").and_then(|v| v.as_str()).unwrap_or("").to_string());
            row.insert(
                "status_code".to_string(),
                json.get("status_code").and_then(|v| v.as_i64()).unwrap_or(0).to_string(),
            );
